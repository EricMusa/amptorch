...
# trainer.train()
# config = new_config(EarlyStopping(patience=30, threshold=0.10))
...
atoms = test_atoms
traj_path = 'ml_opt_active_%02d.traj'
logfile = '-'
...
while True:
    ...
    try:
        ...
        dyn = SciPyFminBFGS(ep.connect_opt_atoms(atoms), trajectory=traj_path % count, logfile=logfile)
        dyn.attach(ep, m=3)
        # dyn.attach(capturer)
