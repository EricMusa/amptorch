...
def sample_images(n_images, traj_sampled=False, seed=None):
    ...
    if traj_sampled:
        ...
        for i in np.random.choice(
            [_ for _ in range(len(traj_indices))], n_images, replace=True
        ):
...
new_sqr_errs = np.power(new_true_energies - new_pred_energies, 2.0)
...
ml_sqr_errs = np.power(ml_true_energies - ml_pred_energies, 2.0)
...
def calculate_metrics(latent_data, pde, metric="cityblock"):
    dmat = squareform(pdist(latent_data, metric=metric))
    inv_dmat = np.power((dmat + np.diag([np.inf for _ in range(len(dmat))])), -1.0)

    point_densities = np.power(inv_dmat.sum(axis=0) / (len(inv_dmat) - 1.0), -1.0)

    probs = np.power(pde(latent_data), -1)

    n_points = 10
    n_nearest = np.array(
        [dmat[i].argsort()[1 : n_points + 1] for i in range(len(dmat))]
    )
    n_nearest_distances = np.array(
        [[dmat[i, j] for j in n_nearest[i]] for i in range(len(dmat))]
    )
    nnn_distance_averages = n_nearest_distances.mean(axis=1)

    inv_nnn_distances = 1.0 / n_nearest_distances
    inv_nnn_distance_averages = inv_nnn_distances.mean(axis=1)
    inv_nnn_distance_averages = np.power(inv_nnn_distance_averages, -1.0)
...
(
    point_densities,
    probs,
    nnn_distance_averages,
    inv_nnn_distance_averages,
) = calculate_metrics(latents, kde)

print("new")
(
    new_point_densities,
    new_probs,
    new_nnn_distance_averages,
    new_inv_nnn_distance_averages,
) = calculate_metrics(new_latents, kde)

print("ml opt")
(
    ml_point_densities,
    ml_probs,
    ml_nnn_distance_averages,
    ml_inv_nnn_distance_averages,
) = calculate_metrics(ml_latents, kde)
...
for ax, metric, new_metric, ml_metric in zip(
    axs[
        used_metrics,
    ],
    metrics[used_metrics, :],
    new_metrics[used_metrics, :],
    ml_metrics[used_metrics, :],
):
