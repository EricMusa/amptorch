...
from staging.ASE_GA.ga_operations import init_ga_connection, relax_atoms_emt, relax_unrelaxed_candidates, generate_relaxed_candidate
import numpy as np
...
class ErrorPredictor():
    ...
    def __init__(self, model, dataset, n_components=10, disable_tqdm=True):
        self.model = model
        self.dataset = dataset
        self.true_energies = np.array([atoms.get_potential_energy() for atoms in self.dataset])

        print('making predictions')
        self.predictions = self.model.predict(self.dataset, disable_tqdm=disable_tqdm)
        ...
        print('principal components calculated, reducing data to %d components' % self.n_components)
        self.reduced_data = self.dreduce(self.latents, self.evecs[:, :self.n_components])
        ...
        self.error_funcs = {
            ...
            'rts': self.root_square_errors,
        }
        print('available errors:', list(self.error_funcs.keys()))
        self.stats = {}
        self.error_gpr = GaussianProcessRegressor()
    ...
    def condition_error_gpr(self):
        ...
        standard_metrics = np.vstack([self.standardize(v, k) for k, v in self.metrics.items()]).T
        standard_errors = np.vstack([self.standardize(v, k) for k, v in self.errors.items()]).T
        self.error_gpr.fit(standard_metrics, standard_errors)
        print('ErrorPredictor ready!')
    
    def predict(self, atoms, disable_tqdm=True):
        predictions = self.model.predict(atoms, disable_tqdm=disable_tqdm)
    ...
    def calculate_metrics(self, latent_points):
        return {name: metric_func(latent_points) for name, metric_func in self.metric_funcs.items()}
    ...
    def n_nearest_neighbors(self, points, n_neighbors=10,):
    ...
    def latent_density(self, points):
        dists = cdist(points, self.latents)
        dists.sort(axis=1)
        if np.all(dists[:, 0] == 0.):
            dists = dists[:, 1:]
        return np.power(dists, -1.).sum(axis=1)
    
    def n_nearest_neighbors_inverse(self, points, n_neighbors=10,):
        n_neighbors = n_neighbors or self.n_neighbors
        dists = cdist(points, self.latents)
        dists.sort(axis=1)
        if np.all(dists[:, 0] == 0.):
            dists = dists[:, 1:]
        return np.power(dists[:, :n_neighbors], -1.).sum(axis=1)
    
    def absolute_errors(self,):
        return np.abs(self.pred_energies - self.true_energies)

    def root_square_errors(self,):
        return np.power(np.power(self.pred_energies - self.true_energies, 2.), .5)
...
population_size = 20
population, da, comp, pairing, mutations, data = init_ga_connection(db_file, population_size)
...
for i in range(n_initial):
    ...
    if not os.path.isfile(label + '.traj'):
        relax_atoms_emt(sg.get_new_candidate(), trajectory=label + '.traj', 
                        logfile='-', max_steps=25)
...
config = new_config(EarlyStopping(patience=50, threshold=.10))
config['dataset']['raw_data'] = images[:1]
trainer = AtomsTrainer(config)
trainer.load_pretrained('C:\\Users\\ericm\\Documents\\Research\\Code\\Amptorch\\staging\\ASE_GA\\checkpoints\\2021-02-28-00-19-59-test')
...
n_components = 10
ep = ErrorPredictor(trainer, images, n_components=n_components, disable_tqdm=False)
print('ErrorPredictor made')
finish_time = time.time()
print('preparation finished in %0.3f seconds, beginning interesting/interactive phase' % (finish_time - start_time))

test_atoms = sg.get_new_candidate()
results = ep.predict([test_atoms])
print(results[0], results[2], results[3], results[4])
# calc = AMPtorch(trainer)
# test_atoms.set_calculator(calc)
# ml_traj = 'ml_test.traj'
# dyn = SciPyFminCG(test_atoms, trajectory=ml_traj, logfile='-')
