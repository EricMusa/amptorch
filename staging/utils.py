...
class FeatureScaler:
    ...
    def norm(self, data_list, disable_tqdm=False):
        for data in tqdm(
            ...
            # timed("fps scaled", st, len(element_fp))
            if self.forcetraining:
                st_outer = time.time()
                # print("atomic_numbers", atomic_numbers.size(), atomic_numbers[0])
                # print("data.fingerprint.shape", data.fingerprint.shape)
                base_atoms = torch.repeat_interleave(
                    atomic_numbers, data.fingerprint.shape[1]
                )
                # print("base_atoms", base_atoms.size(), base_atoms[0])
                fp_idx = data.fprimes._indices()[0]
                # print("fp_idx", fp_idx.size(), fp_idx[0])
                fp_idx_to_scale = fp_idx % data.fingerprint.shape[1]
                # print("fp_idx_to_scale", fp_idx_to_scale.size(), fp_idx_to_scale[0])
                ...
                # timed("fpprimes scaled", st_outer, len(element_idx))
        # for t, evs in times.items():
        #     print(t, np.mean(evs[0]), np.std(evs[0]))
        #     print(t, evs[1] if any(evs[1]) else None)
        return data_list
